...
#include <ccan/filecnt/filecnt.c>
...
int main(void)
{
	...
	ok1(filecnt_close(h) == 0);
	ok1(!h->nm && !h->tmp && h->fd == -1 && !h->cnt);
	ok1(filecnt_open(h, "t") == 8193 && h->cnt == 8193);

	ok1(filecnt_zero(h) == 0 && h->cnt == 0);
	if (stat("t", &s) == -1) err(1, "stat");
	ok1(s.st_size == 0 && s.st_blocks == 0);
	...
	ok1(filecnt_open(h, "foo/bar/baz") == 0);
	ok1(h->cnt == 0 && strcmp(h->nm, "foo/bar/baz") == 0 && strcmp(h->tmp, "foo/bar/.baz") == 0);
	ok1(filecnt_close(h) == 0);

	ok1(filecnt_open(h, "foo") == -1 && errno == EINVAL);
	ok1(!h->nm && !h->tmp && h->fd == -1 && !h->cnt);

	if (chmod("t", 0) == -1) err(1, "chmod");
	ok1(filecnt_open(h, "t") == -1 && errno == EACCES);
	ok1(!h->nm && !h->tmp && h->fd == -1 && !h->cnt);
	...
	ok1(filecnt_inc(h) == -1 && errno == EBADF);
	ok1(filecnt_zero(h) == -1 && errno == EINVAL);

	if (unlink(".t") == -1 && errno != ENOENT) err(1, "unlink");
	if (unlink(".u") == -1 && errno != ENOENT) err(1, "unlink");
	if (symlink(".t", ".u") == -1) err(1, "symlink");
	if (symlink(".u", ".t") == -1) err(1, "symlink");
	ok1(filecnt_swap(h) == -1 && errno == ELOOP);

	close(fake);
	ok1(filecnt_close(h) == -1 && errno == EBADF);

	h->fd = save;
	ok1(filecnt_free(h) == 0);
